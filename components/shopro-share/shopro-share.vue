...
export default {
	...
	methods: {
		...
		getPosterFormatter() {
			const that = this;
			let data = {};
			console.log(that.configShare)
			switch (this.posterType) {
				case 'index':
					that.shareDetail.query = "vipid=" + that.userInfo.id
					data = {
						backgroundImage: that.configShare.vip_poster_bg,
						drawArray: [
							{
								name: '用户昵称',
								type: 'text',
							...
							// #ifdef MP-WEIXIN
							{
								name: 'wxCode',
								type: 'image',
								url: `${that.$API_URL}wechat/wxacode?path=pages/index/vip/vip&scene=${that.shareDetail.query}`,
								alpha: 1,
								dy: 560,
								isBgCenter: true,
								dWidth: 180,
					...
					break;
				case 'user'://拉新二维码  附带uid  新用户扫码登陆添加uid
					that.shareDetail.query = "uid=" + that.userInfo.id
					data = {
					...
					break;
				case 'goods'://商品分享二维码, 若果是分销商  传分销商id, 普通用户不穿参
					var user = uni.getStorageSync("userInfo")
					console.log(user,that.posterInfo.id)
					// if(user.is_dealer == 1){
						that.shareDetail.query = "fxid=" + user.id + ",goodsId=" + that.posterInfo.id
					// }else{
					// 	that.shareDetail.query = "goodsId=" + that.posterInfo.id
					// }
					data = {
						...
						drawArray: [
							...
							// #ifdef MP-WEIXIN
							{
								name: 'wxCode',
								type: 'image', //微信小程序码
								url: `${that.$API_URL}wechat/wxacode?path=pages/goods/detail&scene=${that.shareDetail.query}`,
								alpha: 1,
								dx: 522,
								dy: 911,
								dWidth: 110,
					...
					break;
				case 'groupon':
					that.shareDetail.query += ",邀请uid=" + that.userInfo.id
					data = {
