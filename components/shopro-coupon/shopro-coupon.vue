		...
		<view class="coupon-wrap" :style="gradientColor" :class="{ 'gray-wrap': state === 3 }">
			<view class="coupon-item u-flex u-row-between">
				...
				<view class="coupon-right u-flex-col">
					<button class=" get-btn" :style="{ color: btnTextColor }" v-if="state === 0" @tap.stop="getCoupon">
						<span v-if="type == 0">立即领取</span>
						<span v-if="type == 1">立即兑换</span>
					</button>
					...
					<view class="surplus-num" :style="{ color: color }" v-if="state === 0">
						<span v-if="type == 0">仅剩{{ stock }}张</span>
						<span v-if="type == 1">{{couponData.score}} 积分</span>
					</view>
				</view>
			</view>
		</view>
