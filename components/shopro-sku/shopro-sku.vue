	...
	<u-popup v-model="showModal" :closeable="true" mode="bottom" close-icon-pos="top-right" border-radius="30" v-if="goodsInfo.sku_price">
		<view class="shop-modal page_box" :style="goodsInfo.is_sku == 0 ? 'height:700rpx' : ''">
			...
			<view class="btn-box foot_box u-flex u-row-between" v-if="buyType === 'cart' || buyType === 'buy'">
				<button class="u-reset-button cu-btn save-btn buy-btn" v-if="(activityRules && activityRules.status === 'ing') || !goodsInfo.activity_type" @tap="confirm">确认</button>
				<button class="u-reset-button cu-btn cancel-btn buy-btn" v-if="activityRules && activityRules.status !== 'ing' && goodsInfo.activity_type" @tap="showModal = false">
					确定
				</button>
			</view>
	...
	methods: {
		...
		confirmBuy() {
			...
			if (this.confirmSku()) {
				...
				that.jump('/pages/order/confirm?num='+that.goodsInfo.goodsNum, {
					goodsList: confirmGoodsList,
					from: 'goods',
					orderType: that.goodsInfo.duihuanType?'code&'+that.goodsInfo.goodsNum:that.goodsType,
		...
		confirmSku() {
			let that = this;
			if (that.goodsInfo.stock == 0 || that.goodsInfo.stock < that.goodsNum) {
				that.$u.toast('库存不足');
				that.showModal = false;
				return false;
			} else {
