	...
	<view class="address-wrap">
		<u-form :model="model" :rules="rules" ref="uForm" :errorType="errorType">
			...
			<view class="default-box u-flex u-row-between">
				...
				<u-switch v-model="model.is_default" activeColor="#ff9c00" size="40"></u-switch>
			</view>
...
export default {
	...
	methods: {
		...
		editAddress() {
			let that = this;
			console.log("微信解析")
			// ?address=' . $address . '&key=6b3fb3196bad793207ddda213e024a51'
			// http://restapi.amap.com/v3/geocode/geo?address=%27%20%E6%B2%B3%E5%8D%97%E7%9C%81%E9%83%91%E5%B7%9E%E5%B8%82%E9%87%91%E6%B0%B4%E5%8C%BA%27&key=6b3fb3196bad793207ddda213e024a51
			wx.request({
			  url: 'https://restapi.amap.com/v3/geocode/geo', //仅为示例，并非真实的接口地址
			  data: {
			    address: that.model.area_text.replace("-","").replace("-","") + that.model.address,
			    key: '1a9cd68da9cb0a7caf1dac3028f451ad',
			  },
			  method:'GET',
			  success (res) {
				console.log(res.data.geocodes[0].location.split(",")[0],res.data.geocodes[0].location.split(",")[1])
			    that.model.latitude = res.data.geocodes[0].location.split(",")[0]
			    that.model.longitude = res.data.geocodes[0].location.split(",")[1]
				that.$http(
					'address.edit',
					{
						...that.model,
