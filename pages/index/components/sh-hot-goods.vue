		...
		<view class="u-waterfall" v-if="goodsType === 1">
			<view id="u-left-column" class="u-column">
				<view class="goods-item u-m-b-16 u-flex u-row-center u-col-center" v-for="leftGoods in leftList" :key="leftGoods.id">
					<shopro-goods-card
						:detail="leftGoods"
						:need_score="leftGoods.need_score"
			...
			<view id="u-right-column" class="u-column">
				<view class="goods-item  u-m-b-16 u-flex u-row-center u-col-center" v-for="rightGoods in rightList" :key="rightGoods.id">
					<shopro-goods-card
						:detail="rightGoods"
						:need_score="rightGoods.need_score"
	...
	computed:{
		storeId(){
			if(this.detail.shopId){
				this.listParams.store_id = this.detail.shopId
				this.getGoodsList()
			}
			return this.detail;
		}
	...
	methods: {
		...
		getGoodsList() {
			...
			if(this.listParams.store_id){
				that.$http('user.shopList', this.listParams).then(res => {
					if (res.code === 1) {
						console.log(res,'goods')
						this.lastPage = res.data.goods_list.last_page;
						this.total = res.data.goods_list.total;
						this.perPage = res.data.goods_list.per_page;
						this.isRefresh = false;
						that.goodsList = [...that.goodsList, ...res.data.goods_list.data];
						that.tempList = res.data.goods_list.data;
						that.splitData();
					}
				});
			}else{
