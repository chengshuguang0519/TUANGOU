	...
	<view class='shop titleBackground'>
	    ...
	    <view class='listTab'>
	       <!-- <view class='tabClass'>
			...
			<view v-if="shop" class="shopDetail" style="line-height: 60rpx;padding: 20rpx 32rpx;">
				<view class="">
					<span @tap="goMap('/pages/order/express/store-address', { goodsId: '', lat:shop.latitude , lng:shop.longitude , storeId: shop.id , name: shop.name })">地址: {{shop.province_name}}{{shop.city_name}}{{shop.area_name}}{{shop.address}}</span>
					<image src="/static/images/you.png" mode=""></image>
				</view>
				<view class="">
					<span @tap="playTel(shop.phone)">电话: {{shop.phone}}</span>
					<image src="/static/images/you.png" mode=""></image>
				</view>
			</view>
			<view class="tuijian" style="width: 750rpx;height: 88rpx;line-height: 88rpx;text-align: center;margin: 0 auto;">
				<!-- 推荐商品 -->
				<sh-hot-goods  :detail="gouwuche"></sh-hot-goods>
			</view>
	        <!-- <view class='list'>
...
<script>
	...
	import { chooseLocation } from './openMap.js'
	export default {
		components:{
			shHotGoods
		},
		...
		onLoad(options) {
			var _this = this;
			_this.gouwuche.shopId = options.id
			...
			goMap(path, parmas) {
				...
				wx.getLocation({
				 ...
				 success (res) {
				   ...
				   wx.openLocation({
				     latitude:Number(parmas.lat),
				     longitude:Number(parmas.lng),
					 name:parmas.name,
				     scale: 18,
				   })
				 }
				...
				// chooseLocation()
				// // Map.openMap(114.28808,35.76129,'parmas.name')
				// this.$Router.push({
				// 	path: '/pages/index/shop/map/map',
				// });
			},
		}
	}
