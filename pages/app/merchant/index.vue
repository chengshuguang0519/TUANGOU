...
<template>
	<view class="merchant-wrap">
		<view class="shopinfo-box">
			...
			<!-- 商家信息 -->
			<view class="user-head u-flex u-row-between" style="position: relative;">
				<view class="shop-info">
					...
					<view class="btns" style="position: absolute; top: 50%; right: 0;">
						<button @tap="goUser()" class="u-reset-button merchant-btn" style="margin-bottom: 20rpx;">切换个人版</button>
						<button @tap="goziti()" class="u-reset-button merchant-btn">自提点管理</button>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="order-list" v-for="order in storeOrderList" :key="order.order_sn" @tap="jump('/pages/app/merchant/detail', { orderId: order.id })">
			...
			<view class="<strong>order-bottom</strong> u-flex">
				<view>
					<text class="total-price-title">积分：</text>
					<text class="total-score">{{ order.score_fee }}</text>
				</view>
				<view>
...
export default {
	...
	methods: {
		goziti(){
			uni.navigateTo({
				url:'/pages/zitidian/zitidian?id=' + this.storeDetail.id,
			})
		},
		goUser(){
			uni.reLaunch({
		...
		getStoreOrder() {
			...
			console.log(this.custom.toString());
			that.$http('store.order', {
				date_type: that.filter1,
				date: that.custom.toString(),
				type: that.filter2,
