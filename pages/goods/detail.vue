	...
	<view class="detail-wrap ">
		...
		<view class="detail_box" v-else>
			<view class="detail-content">
				<!-- 轮播 -->
				<u-swiper :height="750" borderRadius="0" autoplay="false" :list="goodsInfo.images" :videofile="videofile" indicator-pos="bottomRight" mode="number" @click="onGoodsSwiper"></u-swiper>

				<!-- 价格卡片组 -->
				<sh-price-card v-if="goodsInfo.id" :detail="goodsInfo" :type="detailType" @change="getActivityRules"></sh-price-card>

				<!-- 标题 -->
				<view class="shopTitle u-m-b-10 title-box u-skeleton-fillet">
					...
					<view class="shop" @tap="goShop(goodsInfo.store_id)" v-if="goodsInfo.store_id != 0">
						<view class="img"><image src="/static/images/shopIcon.png " mode=""></image></view>
						<view class="name">进入店铺</view>
					</view>
				</view>
				...
				<view class="detail-foot_box safe-area-inset-bottom  u-flex">
					...
					<view class="left u-flex">
						...
						<view class="tools-item u-flex-col u-row-center u-col-center" @tap="goHome">
							...
							<image class="tool-img shopro-selector-circular" src="/static/images/home.png" mode=""></image>
					...
					<view class="detail-right">
						<!-- 积分按钮 -->
						<view class="detail-btn-box" v-if="detailType === 'score'"><button class="u-reset-button  score-btn" @tap="goPay(1)">立即兑换</button></view>
						<view v-if="detailType !== 'score'">
							<!-- 正常按钮 -->
							<view class="detail-btn-box u-flex u-row-around" v-if="!goodsInfo.activity_type">
								<button v-if="!type" class=" u-reset-button tool-btn add-btn" @tap="addCart">加入购物车</button>
								<button :class=" type?'u-reset-button seckill-btn':'u-reset-button tool-btn pay-btn' " @tap="goPay">立即购买</button>
							</view>
							<!-- 活动按钮 -->
			...
			<!-- 	分享组件 -->
			<shopro-share ref="pupopImg" v-model="showShare" :shareDetail="shareInfo" :posterInfo="goodsInfo" :posterType="'goods'"></shopro-share>
			<!-- 登录 -->
			<!-- v-if="authType" -->
...
export default {
	...
	async onLoad(options) {
		...
		if(that.$Route.query.id){
			console.log(that.$Route.query.id,'id')
		}else{
			...
			console.log(obj,'obj')
			if(obj.fxid){
				uni.setStorageSync('topid',obj.fxid)
				...
				uni.setStorageSync('topid','0')
			}
			if(obj.goodsId){
				setTimeout(()=>{
					that.getGoodsDetail(obj.goodsId)
				},1)
			}
		}
		if(options.type){
	...
	methods: {
			...
			return new Promise((resolve, reject) => {
				that.$http('goods.scoreDetail', {
					...
					if (res.code === 1) {
						...
						that.videofile = res.data.videofile
						resolve(res.data);
					}
					if (res.code == 0) {
						resolve(false);
		...
		// 立即购买
		goPay(type) {//参数为true,积分兑换商品  false,兑换码兑换
			if (this.isLogin) {
				this.buyType = 'buy';
				this.showSku = true;
				if(type == 1){
					...
					if(this.type){
						this.goodsInfo.duihuanType = this.type
					}
					if(this.goodsNum){
						this.goodsInfo.goodsNum = this.goodsNum
					}
				}
			} else {
				this.$store.dispatch('showAuthModal');
...
.detail-foot_box {
	...
	.detail-btn-box {
		...
		.pay-btn {
			...
			// background: linear-gradient(90deg, rgba(255, 156, 0, 1), rgba(255, 206, 108, 1));
			background: #ff9900;
