	...
	<view class="order-wrap">
		...
		<view class="u-m-b-10">
			...
			<view class="goods-list" v-for="g in perGoodsList" :key="g.sku_price_id">
				...
				<view class="express-bottom" @tap="checkProtocol" style="padding: 0 25rpx;">
				...
				<view class="logistic item-list u-flex u-row-between" @tap="onSelExpressType(g)">
					...
					<view class="u-flex">
						<view class="detail">{{ getCurGoodsExpress(g) }}:{{skhStore.name?skhStore.name:storeInfo.name}}</view>
						<u-icon name="arrow-right" size="28" color="#bfbfbf"></u-icon>
					</view>
				</view>
			</view>
			...
			<block v-if="perGoodsList.length">
				...
				<view class="score item-list u-flex u-row-between" v-show="orderPre.score_amount>0">
					<view class="u-flex"><text class="item-title">积分</text></view>
					<view class="price">-{{ orderPre.score_amount || 0 }}积分</view>
				</view>
				<!-- 金额明细 -->
				<view class=" u-flex u-row-between item-list border-top ">
					<view class="item-title">商品金额</view>
					<view class="u-flex">
						<text class="price">￥{{ orderPre.goods_amount || '0.00' }}</text>
					</view>
				</view>
				<view class="express-bottom Score u-m-b-10" @tap="checkScore" >
					<view class="protocol">
						<text>是否使用积分</text>
						<text>(当前可用积分:{{score}})</text>
					</view>
					<u-checkbox active-color="#f0c785" :value="isScore"></u-checkbox>
				</view>
				<view v-if="orderPre.vip_fee>0" class=" u-flex u-row-between item-list border-top u-m-b-10">
					<view class="item-title">VIP折扣</view>
					<view class="u-flex">
						<text class="price">￥{{ orderPre.vip_fee || '0.00' }}</text>
					</view>
				</view>
						...
						<view class="" v-else>
							...
							>
								<view class="">
									...
									<text class="address">{{ skhStore.name ? skhStore.name : storeInfo ? storeInfo.name : '暂无自提点' }}</text>
									<u-icon name="arrow-right" size="28" color="#999"></u-icon>
								</view>
								<view class="address-location u-flex-col u-col-center">
									<image class="location-img" :src="$IMG_URL + '/imgs/order/e1.png'" mode=""></image>
									<view class="location-text">距您{{ skhStore.name ? skhStore.distance_text : storeInfo ? storeInfo.distance_text : 0 }}</view>
								</view>
							</view>
...
export default {
	...
	data() {
		return {
			...
			isScore:true,//使用积分
			selfPhone: uni.getStorageSync('userInfo').mobile, //编辑手机号
			getFocus: false, //获取焦点。
			checkType: '自提',
	...
	onShow() {
		...
		uni.$once('SELECT_STORE', res => {
			let dizhi = JSON.parse(res.storeInfo)
			this.storeInfo = dizhi;
			this.skhStore = dizhi;
			this.storeList[0] = dizhi
		});
		// 监听地址
	...
	methods: {
		...
		getPre(type) {
			...
			that.$http('order.pre', {
				...
				if (res.code === 1) {
					...
					res.data.new_goods_list.forEach(function(g){
						that.onSelExpressType(g,1)
						that.getCurGoodsExpress(g)
						if(type){
							
						}else{
							that.saveExpressType()
						}
					})
		...
		subOrder() {
			...
			that.goodsList[0].store_id = that.skhStore.name ? that.skhStore.id : that.storeInfo.id
			that.goodsList[0].dispatch_phone = that.selfPhone
			that.$http(
				'order.createOrder',
				{
					goods_list: that.goodsList,
					...
					buy_type: that.grouponBuyType,
					groupon_id: that.grouponId,
					pid:that.goodsList[0].goods_id == uni.getStorageSync('goodsId')?uni.getStorageSync('topid'):0,
					// pid:uni.getStorageSync('topid')?uni.getStorageSync('topid'):0,
					is_use_score:this.isScore
				},
				'提交中...'
			).then(res => {
				...
				if (res.code === 1) {
					...
					that.$Router.replace({
						...
						query: {
							...
							goodsid :that.goodsList[0].goods_id
						}
					});
				}
			}).finally(()=>{
				if(that.goodsList[0].goods_id == uni.getStorageSync('goodsId')){
					console.log('1111')
		...
		// 显示配送方式弹窗
		async onSelExpressType(goods,show) {
			if(show){
...
.Score{
	...
	font-size: 28rpx;
	border-top: 1rpx solid rgba(223, 223, 223, 0.5);
}
</style>
