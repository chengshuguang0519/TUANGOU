...
<template>
	<view class="map-box">
		...
		<map id="map" :style="{ width: '750rpx', height: 600 + 'px' }" :latitude="latitude" :longitude="longitude" :markers="covers" show-location @tap="onMap"></map>
...
export default {
	...
	onLoad(options) {
		...
		console.log(this.$Route.query,'map')
	...
	methods: {
		...
		onSave() {
			if (this.storeId) {
				...
				uni.$emit('SELECT_STORE', { skhStore: JSON.stringify(this.storeInfo) });
				console.log(JSON.stringify(this.storeInfo),'选择地址')
				this.$Router.back();
			} else {
				this.$u.toast('请选择门店');
			}
