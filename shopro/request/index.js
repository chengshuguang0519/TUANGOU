...
export default function http(
	...
	shoproRequest.interceptor.request((config, cancel) => {
		...
		let province_name = uni.getStorageSync('province');
		let city_name = uni.getStorageSync('city');
		shoproRequest.setConfig(config => {
				config.header.provinceName = encodeURIComponent(province_name)
				config.header.cityName = encodeURIComponent(city_name)
		})
		if (api.auth && !token) {
